...
ofstream myfile;
myfile.open ("Nx_"+std::to_string(Nx)+"_J2_"+std::to_string(J2)+"_hh_"+std::to_string(hh)+".dat");
...
for( auto j : range1(N) ) 
        ...
        auto Spjop = op(sites,"S+",j);
        auto Smjop = op(sites,"S-",j);

        //take an inner product 
        auto szj = elt(bra*Szjop*ket);
        auto spj = elt(bra*Spjop*ket);
        auto smj = elt(bra*Smjop*ket);
